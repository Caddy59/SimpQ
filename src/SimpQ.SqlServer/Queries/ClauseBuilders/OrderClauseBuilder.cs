...
/// </param>
public class OrderClauseBuilder(SqlServerQueryOperator sqlServerQueryOperator)  {
    ...
    private string GetColumns<TEntity>(IReadOnlyCollection<Order>? order, bool isKeysetPagination = false) where TEntity : IReportEntity, new() {
        var allowedColumns = QueryHelper.GetAllowedColumnsToOrder<TEntity>();
    ...
    private string GetDefaultOrderClause<TEntity>() where TEntity : IReportEntity, new() {
        var defaultOrders = QueryHelper.GetDefaultColumnsToOrder<TEntity>();
